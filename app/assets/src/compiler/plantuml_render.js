let plantuml = (() => {
    let initialize = async (cheerpjPath = "/app/assets/lib") => {
    ...
    let renderPng = (pumlContent) => {
        return new Promise((resolve) => {
            let renderingStartedAt = new Date();
            let resultFileSuffix = renderingStartedAt.getTime().toString();
            cjCall("com.plantuml.wasm.v1.Png", "convert", "light", `/files/result-${resultFileSuffix}.png`, pumlContent).then((result) => {
                ...
                if (obj.status === "ok") {
                    cjFileBlob(`result-${resultFileSuffix}.png`).then((blob) => {
                        let transaction = cheerpjGetFSMountForPath("/files/").dbConnection.transaction("files", "readwrite");
